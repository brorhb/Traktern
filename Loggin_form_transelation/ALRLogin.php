    ...
    public function shortcode(){
        ...
        if ( is_user_logged_in() ) {
            ...
            $html = sprintf(
                ...
                $this->prefix,
                __('Du er allerede logget inn', ZM_ALR_TEXT_DOMAIN ), // Text
                wp_logout_url( site_url() ), // URL
                __('Logg ut', ZM_ALR_TEXT_DOMAIN ), // Link text
                __('Logg ut', ZM_ALR_TEXT_DOMAIN ) // Link title text
            );
    ...
    public function getLogInForm(){
        ...
        $links_html = $this->_zm_alr_html->buildFormHtmlLinks( array(
            $this->prefix . '_not_a_member' => array(
                ...
                'text' => __( 'Er du allerede medlem?', ZM_ALR_TEXT_DOMAIN ),
                ),
            $this->prefix . '_lost_password_url' => array(
                'href' => wp_lostpassword_url(),
                'class' => '',
                'text' => __( 'Glemt Passord',ZM_ALR_TEXT_DOMAIN )
                )
            ), $this->prefix );
        ...
        $fields_html = $this->_zm_alr_html->buildFormFieldsHtml( array(
            $this->prefix . '_user_name' => array(
                'title' => __( 'Brukernavn', ZM_ALR_TEXT_DOMAIN ),
                'type' => 'text',
                'extra' => 'autocorrect="none" autocapitalize="none"'
                ),
            $this->prefix . '_password' => array(
                'title' => __( 'Passord', ZM_ALR_TEXT_DOMAIN ),
                'type' => 'password',
                'extra' => 'autocorrect="none" autocapitalize="none"'
                ),
            $this->prefix . '_keep_me_logged_in' => array(
                'title' => __( 'Forbli logget inn?', ZM_ALR_TEXT_DOMAIN ),
                'type' => 'checkbox'
                ),
            $this->prefix . '_submit_button' => array(
                'title' => __( 'Logg inn', ZM_ALR_TEXT_DOMAIN ),
                'type' => 'submit'
                )
            ), $this->prefix );
    ...
    public function loginSubmit( $user_login=null, $password=null, $is_ajax=true ) {
        ...
        elseif ( validate_username( $args['user_login'] ) ){
            ...
            if ( username_exists( $args['user_login'] ) ){
                ...
                $status = $this->_zm_alr_helpers->status('Brukernavnet_finnes_ikke');

            }

        } else {

            $status = $this->_zm_alr_helpers->status('Ugyldig_brukernavn');
